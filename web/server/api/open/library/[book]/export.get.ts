/**
 * Open API: stream knowledge-base ZIP (Bearer required via middleware).
 * Same limits as console export: 3/min + global single-flight.
 */
import {
  contentDisposition,
  createBookZipStream,
...
export default defineEventHandler(async (event) => {
  assertRateLimit(clientKey(event, 'open-export'), 3, 60_000)
